        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-3">
            ...
            <div class="relative">
              ...
              <!-- Export Menu -->
              <div v-if="exporting" class="absolute right-0 mt-2 w-48 rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                <button
      ...
      <div v-else-if="supplier" class="space-y-8">
          ...
          <div class="flex items-start justify-between">
            <div class="flex items-start space-x-4">
              ...
              <div>
                ...
                <div class="mt-2">
                  <span :class="['inline-flex rounded-full px-2 py-1 text-xs font-medium', getStatusInfo(supplier.kyb_status).color]">
                    {{ getStatusInfo(supplier.kyb_status).label }}
                  </span>
                  <span v-if="supplier.grade" class="ml-2 inline-flex rounded-full bg-purple-100 px-2 py-1 text-xs font-medium text-purple-800">
          ...
          <div class="mt-6">
            ...
            <div class="mt-2 h-2 rounded-full bg-gray-200">
              <div
                ...
                :style="{ width: `${supplier.completion_percentage}%` }"
              ></div>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div class="space-y-4">
            ...
            <!-- Under Review -->
            <div class="flex items-start space-x-3">
              <div :class="['flex h-8 w-8 items-center justify-center rounded-full', supplier.kyb_status === 'under_review' || supplier.kyb_status === 'approved' ? 'bg-blue-100' : 'bg-gray-100']">
                <svg :class="['h-4 w-4', supplier.kyb_status === 'under_review' || supplier.kyb_status === 'approved' ? 'text-blue-600' : 'text-gray-400']" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div v-else class="space-y-3">
            <div v-for="document in documents" :key="document.id" class="flex items-center justify-between rounded-lg border border-gray-200 p-4">
              <div class="flex items-center space-x-3">
              ...
              <div class="flex items-center space-x-3">
                <span :class="['inline-flex rounded-full px-2 py-1 text-xs font-medium', getDocumentStatusInfo(document.status).color]">
        ...
        <!-- Next Steps -->
        <div v-if="supplier.kyb_status !== 'approved'" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-start space-x-3">
              <div class="flex h-6 w-6 items-center justify-center rounded-full bg-blue-100">
                <span class="text-xs font-medium text-blue-600">{{ supplier.kyb_status === 'pending' ? '3' : '1' }}</span>
              </div>
              <div>
                <p class="text-sm font-medium text-gray-900">Wait for review</p>
                <p class="text-sm text-gray-500">Our compliance team will review your application within 2-3 business days.</p>
              </div>
            </div>
          </div>
        </div>
