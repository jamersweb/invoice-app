        ...
        <div class="space-y-6">
            ...
            <div class="card p-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <DarkInput v-model="search" placeholder="Company, email, or legal name..." icon="search" />
                    </div>
                    <div>
            ...
            <div class="card overflow-hidden p-0">
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-dark-border">
                                <th
                                    class="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-dark-text-secondary text-left">
                                    Company</th>
                                <th
                                    class="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-dark-text-secondary text-left">
                                    Contact</th>
                                ...
                                <th
                                    ...
                                    Submitted</th>
                                <th
                                    class="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-dark-text-secondary text-right">
                                    Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-dark-border">
                            <tr v-for="supplier in suppliers.data" :key="supplier.id"
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-dark-text-secondary">{{ supplier.contact_phone || '-' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                ...
                <div v-if="suppliers.last_page > 1"
                    class="border-t border-dark-border p-4 flex items-center justify-between">
                    <div class="text-sm text-dark-text-secondary">
                        Showing {{ suppliers.from }} to {{ suppliers.to }} of {{ suppliers.total }} suppliers
                    </div>
                    <div class="flex items-center gap-2">
                        <Link v-for="(link, i) in suppliers.links" :key="i" :href="link.url || '#'"
                            class="px-3 py-1 text-sm rounded-md border border-dark-border transition-colors" :class="[
