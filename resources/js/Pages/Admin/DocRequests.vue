        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 lg:col-span-2">
          <div class="mb-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <input type="number" v-model.number="supplierId" placeholder="Supplier ID" class="rounded-lg border border-gray-300 px-3 py-2 text-sm" />
              ...
              <tbody>
                <tr v-for="r in items" :key="r.id" class="border-t">
                  ...
                  <td class="py-2 text-right">
                    <button v-if="!r.resolved_at" @click="resolveReq(r.id)" class="text-indigo-600 hover:underline">Mark resolved</button>
                  </td>
                </tr>
                <tr v-if="items.length===0"><td colspan="6" class="py-6 text-center text-gray-500">No requests</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <div class="space-y-3">
            <div>
              ...
              <input type="number" v-model.number="form.supplier_id" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
            <div>
            ...
            <div class="pt-2">
              <button @click="save" class="rounded-lg bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700">Create</button>
            </div>
          </div>
        </div>
      </div>
