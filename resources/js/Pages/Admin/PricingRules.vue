            ...
            <table class="min-w-full text-left text-sm">
              ...
              <tbody>
                <tr v-for="r in rules" :key="r.id" class="border-t">
                  ...
                  <td class="py-2"><button class="text-indigo-600 hover:underline" @click="toggle(r.id,'is_active')">{{ r.is_active ? 'Yes' : 'No' }}</button></td>
                  <td class="py-2 text-right"><button class="text-red-600 hover:underline" @click="remove(r.id)">Delete</button></td>
                </tr>
                <tr v-if="rules.length===0"><td colspan="6" class="py-6 text-center text-gray-500">No rules</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" v-model.number="form.tenor_min" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Tenor Max</label>
              <input type="number" v-model.number="form.tenor_max" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Amount Min</label>
              <input type="number" v-model.number="form.amount_min" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Amount Max</label>
              <input type="number" v-model.number="form.amount_max" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Base %</label>
              <input type="number" step="0.001" v-model.number="form.base_rate" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">VIP Adj %</label>
              <input type="number" step="0.001" v-model.number="form.vip_adjust" class="mt-1 w-full rounded-lg border border-gray-300 p-2" />
            </div>
